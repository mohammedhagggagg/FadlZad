...
namespace FadlZ.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ChangePassword(ChangePasswordBindingModel model)
        {
            ...
            if (!result.Succeeded)
            {
                return BadRequest(GetErrorResult(result).ToString());
            }
       ...
       [Route("RegisterAdmin")]
        ...
        [HttpPost]
        [Route("Login")]
        public IHttpActionResult Login(LoginBinding model)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    return BadRequest("Not Found this info");
                }
                else
                {
                    SignInManager<ApplicationUser, string> signin =
                        new SignInManager<ApplicationUser, string>(UserManager, Authentication);
                    signin.SignIn(user, false, false);
                    
                    if (UserManager.IsInRole(user.Id, "Client"))
                    {
                        return Ok(  "is authanticated in Client Role");
                    }
                    else if (UserManager.IsInRole(user.Id, "Admin"))
                    {
                        return Ok("is authanticated in Admin Role");
                    }
                    else
                    {
                        return Ok("is authanticated but not in Role");
                    }
